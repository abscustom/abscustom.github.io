...
window.unlockAdminMode = function() {
    ...
    if (!token) {
        token = prompt("🔒 Enter Admin Personal Access Token to enable Live Editing:");
        if (!token) return;
    ...
    if (quickSaveBtn) quickSaveBtn.style.display = 'inline-flex';

    alert("🔓 Admin Mode Unlocked!\n\nYou can now click on card elements to edit them. Click 'Save Quick Edit' in the top bar to save changes live.");
};

// Keyboard Shortcut: Ctrl + Shift + A (Unlock Admin Mode)
...
// Double Click Title/Logo Trigger
document.addEventListener('dblclick', function(e) {
    if (e.target.closest('.hud-btn-logo, .sidebar-main-title, #db-art-header-text')) {
        window.unlockAdminMode();
    }
});
...
document.addEventListener('click', function(e) {
    // Prevent edits if on published site and admin mode is NOT unlocked
    if (window.IS_PUBLISHED && !window.ADMIN_MODE) return;

    if (e.target.closest('#context-gui, #editor, nav, .navbar')) return;

    let targetBox = e.target.closest('.dokkan-card, .db-box, #card-link-container, #card-category-container, #db-link-container, #db-category-container');
    let editType = null;

    if (targetBox) {
        const boxText = targetBox.innerText || "";
        
        if (e.target.closest('#card-link-container, #db-link-container, .db-links-container, .db-link-badge') || boxText.includes('Link Skills')) {
            editType = 'links';
        } else if (e.target.closest('#card-category-container, #db-category-container, .db-cat-btn') || boxText.includes('Categories')) {
            editType = 'categories';
        }
    }
...
window.highlightCardElement = function(element) {
    window.clearCardGlow();
    if (!element) return;
    const outerBox = element.closest('.dokkan-card, .db-box, .sa-block, .active-block, .card-art-canvas, #forms-container') || element;
    if (outerBox) {
...
function makeGUIDraggable() {
    ...
    header.addEventListener('mousedown', function(e) {
        if (e.target.classList.contains('gui-close')) return;
        isDragging = true;
        ...
        function onMouseMove(e) {
            ...
            const maxTop = Math.max(minTop, window.innerHeight - 100);
...
document.addEventListener('click', function(e) {
    ...
    if (target) {
        ...
        if (e.target.closest('#char-name, #char-description, #db-char-title, #db-char-name')) editType = 'identity';
        else if (e.target.closest('#leader-skill, #db-leader-skill')) editType = 'leader';
        else if (e.target.closest('#card-passive-container, .passive-name-display, #db-passive-container, #db-passive-name')) editType = 'passive';
        else if (e.target.closest('#card-link-container, #db-link-container, .db-links-container, .db-link-badge')) editType = 'links';
        else if (e.target.closest('#card-category-container, #db-category-container, .db-cat-btn')) editType = 'categories';
        else if (e.target.closest('#myOverlayImage, #myOverlayVideo, .card-art-canvas, .db-art-box')) editType = 'art';
        else if (e.target.closest('#forms-container, #db-transformations-box, .db-transform-row')) editType = 'forms';
        else if (e.target.closest('#ssr-row, #tur-row, .card-info-thumb, .card-icon, #db-awakenings-box, .db-awaken-row, .db-awaken-divider')) editType = 'icons';
        else if (e.target.closest('table.col, .db-stats-table, #db-stats-box, .db-stat-cards-row')) editType = 'stats';
        else if (e.target.closest('.sa-block, #db-sa-container > div, .dokkan-db-sa-section')) {
            editType = 'sa';
            let clickedBlock = e.target.closest('.sa-block');
            
            if (!clickedBlock) {
                const dbBlock = e.target.closest('#db-sa-container > div, .dokkan-db-sa-section');
                const dbContainer = document.getElementById('db-sa-container');
            ...
            target = clickedBlock || e.target.closest('#db-sa-container > div');
        }
        else if (e.target.closest('.active-block, #db-active-container > div, .dokkan-db-active-section')) {
            editType = 'active';
            let clickedActive = e.target.closest('.active-block');
            
            if (!clickedActive) {
                const dbActive = e.target.closest('#db-active-container > div, .dokkan-db-active-section');
                const dbContainer = document.getElementById('db-active-container');
            ...
            target = clickedActive || e.target.closest('#db-active-container > div');
        }
...
function openContextGUI(mouseX, mouseY, editType, targetElement) {
    ...
    const isAlreadyOpen = (gui.style.display === 'flex' || gui.style.display === 'block');
    const savedLeft = gui.style.left;
    const savedTop = gui.style.top;
    ...
    switch(editType) {
        case 'identity':
            ...
            bodyHTML = `
                ...
                <textarea id="gui-descInput" class="form-control mb-2" style="height:42px;">${document.getElementById('descInput')?.value || ''}</textarea>
                
                <label class="form-label mb-1">Name</label>
                <input type="text" id="gui-nameInput" class="form-control mb-2" value="${document.getElementById('nameInput')?.value || ''}">
        ...
        case 'leader':
            ...
            bodyHTML = `
                ...
                <textarea id="gui-leaderInput" class="form-control mb-2" style="height:90px;">${document.getElementById('leaderInput')?.value || ''}</textarea>
        ...
        case 'stats':
            ...
            bodyHTML = `
                ...
                <input type="number" id="gui-hp-max" class="form-control mb-2" value="${document.getElementById('input-hp-max')?.value || ''}" oninput="document.getElementById('input-hp-max').value=this.value; calcFromMax('hp');">
                
                <label class="form-label mb-1">ATK (Base Max)</label>
                <input type="number" id="gui-atk-max" class="form-control mb-2" value="${document.getElementById('input-atk-max')?.value || ''}" oninput="document.getElementById('input-atk-max').value=this.value; calcFromMax('atk');">
                
                <label class="form-label mb-1">DEF (Base Max)</label>
                <input type="number" id="gui-def-max" class="form-control mb-2" value="${document.getElementById('input-def-max')?.value || ''}" oninput="document.getElementById('input-def-max').value=this.value; calcFromMax('def');">
            `;
            break;
        ...
        case 'passive':
                ...
                <div class="gui-section-box mb-2">
                    <div class="d-flex justify-content-between align-items-center mb-1">
                        <div class="d-flex align-items-center gap-2">
                            ...
                            <label class="form-label m-0">Section ${id} Header</label>
                        </div>
                    ...
                    <input type="text" id="gui-sec-header-${id}" class="form-control mb-2" value="${headerVal}" oninput="updateHeader(${id}, this.value); if(window.currentCardThemeStyle === 'dokkandb') window.syncToDbLayout();">
                    
                    <div id="gui-sec-body-${id}" style="display: ${isCollapsed ? 'none' : 'block'};">
                        <label class="form-label mb-1">Section Effects</label>
                        <textarea id="gui-sec-text-${id}" class="form-control mb-2" style="height:120px; font-size: 13px; line-height: 1.5;" oninput="document.getElementById('input-sec-${id}').value=this.value; updateSection(${id}, this.value); if(window.currentCardThemeStyle === 'dokkandb') window.syncToDbLayout();">${textVal}</textarea>

                        <div class="gui-btn-grid">
                            <button type="button" class="gui-preset-btn" onclick="insertShortcut('gui-sec-text-${id}', ':up:'); document.getElementById('input-sec-${id}').value=document.getElementById('gui-sec-text-${id}').value; updateSection(${id}, document.getElementById('gui-sec-text-${id}').value); if(window.currentCardThemeStyle === 'dokkandb') window.syncToDbLayout();">↑ Up</button>
                            <button type="button" class="gui-preset-btn" onclick="insertShortcut('gui-sec-text-${id}', ':down:'); document.getElementById('input-sec-${id}').value=document.getElementById('gui-sec-text-${id}').value; updateSection(${id}, document.getElementById('gui-sec-text-${id}').value); if(window.currentCardThemeStyle === 'dokkandb') window.syncToDbLayout();">↓ Down</button>
                            <button type="button" class="gui-preset-btn" onclick="insertShortcut('gui-sec-text-${id}', ':ydown:'); document.getElementById('input-sec-${id}').value=document.getElementById('gui-sec-text-${id}').value; updateSection(${id}, document.getElementById('gui-sec-text-${id}').value); if(window.currentCardThemeStyle === 'dokkandb') window.syncToDbLayout();">↓ Y-Down</button>
                            <button type="button" class="gui-preset-btn" onclick="insertShortcut('gui-sec-text-${id}', ':once:'); document.getElementById('input-sec-${id}').value=document.getElementById('gui-sec-text-${id}').value; updateSection(${id}, document.getElementById('gui-sec-text-${id}').value); if(window.currentCardThemeStyle === 'dokkandb') window.syncToDbLayout();">! Once</button>
                            <button type="button" class="gui-preset-btn" onclick="insertShortcut('gui-sec-text-${id}', ':inf:'); document.getElementById('input-sec-${id}').value=document.getElementById('gui-sec-text-${id}').value; updateSection(${id}, document.getElementById('gui-sec-text-${id}').value); if(window.currentCardThemeStyle === 'dokkandb') window.syncToDbLayout();">∞ Inf</button>
                        </div>
                    </div>
                </div>`;
            });
            ...
            bodyHTML = `
                ...
                <input type="text" id="gui-passive-name" class="form-control mb-3" value="${document.getElementById('input-passive-name-sidebar')?.value || ''}">
                ${passiveSectionsHTML}
            `;
            break;
       ...
       case 'sa':
        ...
        case 'art':
            ...
            const isDbModeArt = window.currentCardThemeStyle === 'dokkandb';

            bodyHTML = `
                ${isDbModeArt ? `
                    <label class="form-label mb-1">Card Art Header Tag (DB Mode)</label>
                    <select id="gui-db-unit-tag" class="form-control mb-3" onchange="window.dbUnitTag=this.value; if(window.currentCardThemeStyle === 'dokkandb') window.syncToDbLayout();">
                        <option value="DOKKAN FESTIVAL UNIT" ${(window.dbUnitTag === 'DOKKAN FESTIVAL UNIT' || window.dbUnitTag === undefined) ? 'selected' : ''}>DOKKAN FESTIVAL UNIT</option>
                        <option value="CARNIVAL UNIT" ${window.dbUnitTag === 'CARNIVAL UNIT' ? 'selected' : ''}>CARNIVAL UNIT</option>
                        <option value="LEGENDARY SUMMON UNIT" ${window.dbUnitTag === 'LEGENDARY SUMMON UNIT' ? 'selected' : ''}>LEGENDARY SUMMON UNIT</option>
                        <option value="" ${window.dbUnitTag === '' ? 'selected' : ''}>None / Hidden</option>
                    </select>
                ` : ''}
        ...
        case 'forms':
            ...
            const isDbMode = window.currentCardThemeStyle === 'dokkandb';
            ...
            formCards.forEach((formCard, idx) => {
                ...
                const uploadInputHTML = isDbMode ? `
                    <label class="form-label mb-1">Upload Card Icon / Portrait (DB Mode)</label>
                    <input type="file" class="form-control mb-2" accept="image/*" onchange="guiUploadFormDbThumb(${idx}, event)">
                ` : `
    ...
    if (isAlreadyOpen && savedLeft && savedTop) {
        gui.style.left = savedLeft;
        gui.style.top = savedTop;
        return;
    }
...
window.guiUpdateSAType = function(val) {
    ...
    if (window.currentCardThemeStyle === 'dokkandb') window.syncToDbLayout();
};
...
window.guiSetActiveTypeIcon = function(element, iconSrc) {
    ...
    if (window.currentCardThemeStyle === 'dokkandb') {
...
function bindContextListeners(editType) {
    if (editType === 'identity') {
        ['gui-descInput', 'gui-nameInput', 'gui-dateInput'].forEach(id => {
            ...
            if (el) el.addEventListener('input', () => {
                ...
                if (target) {
                    ...
                    if(window.currentCardThemeStyle === 'dokkandb') { window.syncToDbLayout(); window.enforceDbPipes(); }
                }
            });
        });
    }
    ...
    if (editType === 'passive') {
        ...
        if (el) el.addEventListener('input', () => {
            ...
            if (sidebarInput) sidebarInput.value = el.value;
...
window.enforceDbPipes = function() {
    const dbHeaders = document.querySelectorAll('.db-header, #db-sa-container .sa-title, #db-active-container .active-title');
...
window.guiUploadFormDbThumb = function(idx, event) {
    const file = event.target.files[0];
    if (!file) return;
    const reader = new FileReader();
    reader.onload = function(e) {
