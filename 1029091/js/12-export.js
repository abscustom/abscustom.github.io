...
/* Top Bar Loader & Success Badge Engine */
window.showHudLoader = function(message) {
    const badge = document.getElementById('hud-loading-spinner');
    const spinner = document.getElementById('hud-spinner-icon');
    const check = document.getElementById('hud-check-icon');
    const textEl = document.getElementById('hud-loading-text');
...
window.showHudSuccess = function(message) {
    ...
    if (badge) badge.classList.add('is-success');
...
window.hideHudLoader = function() {
    ...
    const check = document.getElementById('hud-check-icon');
    if (badge) {
        badge.classList.remove('is-visible', 'is-success');
    }
    if (spinner) spinner.style.display = 'block';
    if (check) check.style.display = 'none';
};
...
window.openUploadModal = function() {
    ...
    if (!defaultFolderId) {
        ...
        if (m) defaultFolderId = m[1];
    }

    if (!defaultFolderId) {
        defaultFolderId = Math.floor(1000000 + Math.random() * 9000000).toString();
    }
    ...
    if (idInput) idInput.value = defaultFolderId;
    if (tokenInput) {
...
window.debounceIdCheck = function() {
    ...
    if (!/^\d{7}$/.test(val)) {
        ...
        statusMsg.innerText = "ID must be exactly 7 digits.";
...
window.checkFolderAvailability = async function(id) {
    ...
    try {
        ...
        if (res.status === 200) {
            // Folder exists - PREVENT OVERWRITE
            statusIcon.innerHTML = `
                <svg class="status-icon-x" viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                    <line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line>
                </svg>`;
            ...
            // Rate limit or other error - Fallback allow
...
// ============================================================
// CORE GITHUB PUBLISHER (CALLED BY THE UPLOAD MODAL)
// ============================================================

window.executeGitHubUpload = async function() {
    const folderId = document.getElementById('upload-folder-id').value.trim();
    ...
    if (!folderId || !githubToken) return;

    // Save token if checkbox is ticked, otherwise remove it
    ...
    // AUTO-DETECT HUB FORM LETTER (a, b, c, d, e) WITHOUT EXTRA PROMPTS
    let cleanHubId = "a";
    ...
    try {
        ...
        // PURGE LOADER, HUD, & MODALS FROM PUBLISHED CARDS
        const toRemove = [
            '#uploadGithubBtn', 
            '#topbar-upload-dock-wrap',
            '#icon-picker-modal', 
            '#glass-upload-modal',
            '#glass-quicksave-modal',
...
`;
        ...
        // 3. SCAN ALL REFERENCED RELATIVE ASSETS
        ...
        // 5. UPLOAD ALL FILES IN A SINGLE ATOMIC COMMIT
        ...
        // SHOW UIVERSE ANIMATED GREEN CHECKMARK FOR UPLOAD SUCCESS
...
window.openQuickSaveModal = function() {
    ...
    document.getElementById('quicksave-admin-pass').value = "";
...
window.checkQuickSaveValidity = function() {
    const pass = document.getElementById('quicksave-admin-pass').value;
    ...
    if (pass === "spiderman" && token.length > 15) {
        btn.disabled = false;
    } else {
        btn.disabled = true;
    }
...
window.executeQuickSave = async function() {
    ...
    try {
        ...
        // PURGE LOADER, HUD & UPLOAD DOCK ELEMENTS FROM PUBLISHED CARDS
        const toRemove = [
            '#uploadGithubBtn', 
            '#topbar-upload-dock-wrap',
            '#icon-picker-modal', 
