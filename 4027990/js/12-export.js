...
window.showHudLoader = function(message) {
    ...
    if (textEl && message) {
        ...
        textEl.style.color = "#38bdf8"; // Reset to blue if it was green
...
window.showHudSuccess = function(message) {
    ...
    if (textEl) {
        ...
        textEl.style.color = "#10b981"; // Emerald green
        textEl.style.textShadow = "0 0 10px rgba(16,185,129,0.8)";
    }
    
    // Auto-hide the success overlay after 2 seconds
    setTimeout(() => {
...
window.openQuickSaveModal = function() {
    ...
    window.checkQuickSaveValidity(); // Check validity immediately in case token is saved
};
...
// Ensure button in navbar maps to opening the modal
...
window.executeQuickSave = async function() {
    ...
    try {
        ...
        const toRemove = [
            ...
            '#glass-upload-modal', '#glass-quicksave-modal', '#glass-admin-unlock-modal',
