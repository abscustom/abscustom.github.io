...
window.uploadToGitHub = async function() {
    ...
    try {
        ...
        // 2. CONVERT ALL BLOB & DATA URL IMAGES IN CLONE TO FILES (CUTINS, PORTRAITS, ICONS)
        const allImgsInClone = clone.querySelectorAll('img');
        for (let idx = 0; idx < allImgsInClone.length; idx++) {
            const img = allImgsInClone[idx];
            ...
            if (exportName) {
                ...
                if (blob) {
                    ...
                    const fileName = `img_${idx + 1}_${Date.now().toString(36)}.${ext}`;
                    const path = `${basePath}/images/${fileName}`;
                    if (!fileMap.has(path)) {
                        filesToUpload.push({ path, blob });
                        fileMap.set(path, true);
    ...
    try {
        ...
        for (let idx = 0; idx < allImgsInClone.length; idx++) {
            ...
            if (exportName) {
                ...
                if (blob) {
                    const ext = blob.type.includes('png') ? 'png' : 'jpg';
                    const fileName = `img_quick_${idx + 1}_${Date.now().toString(36)}.${ext}`;
                    const path = `${folderName}/images/${fileName}`;
                    if (!fileMap.has(path)) {
                        filesToUpload.push({ path, blob });
                        fileMap.set(path, true);
                    }
