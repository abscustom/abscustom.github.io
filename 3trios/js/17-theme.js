...
window.restoreThemeOnLoad = function() {
    // If it's a published page, read from the baked script tag variable
    if (window.IS_PUBLISHED && window.currentCardThemeStyle) {
        window.toggleCardTheme(window.currentCardThemeStyle === 'dokkandb');
    } else {
        // In the editor, check localStorage
        const savedTheme = localStorage.getItem('dokkan_selected_theme');
        if (savedTheme === 'dokkandb') {
...
window.syncToDbLayout = function() {
    ...
    try {
        ...
        const dbTitle = document.getElementById('db-char-title');
        const dbName = document.getElementById('db-char-name');
    ...
    try {
        ...
        if (dbLeaderEl && window.formatLeaderSkillQuotes) {
            dbLeaderEl.innerHTML = window.formatLeaderSkillQuotes(leaderText);
        } else if (dbLeaderEl) {
            dbLeaderEl.innerText = leaderText;
        }
    } catch(e) {}

    // 4. Composed Icons, EZA Icons & Card Art Sync
    try {
        const isLR = (window.currentRarity || currentRarity) === 'LR';
        const isSEZA = (window.currentAwakeningMode || currentAwakeningMode) === 'seza';
        ...
        const dbThumbImg = document.getElementById('db-thumb-img');
        if (dbThumbImg && thumbImg) dbThumbImg.src = thumbImg;

        const frameImg = document.querySelector('.card-frame');
        const dbFrameImg = document.getElementById('db-frame-img');
        if (frameImg && dbFrameImg) dbFrameImg.src = frameImg.src;

        const rarityIcon = document.getElementById('main-rarity-icon');
        const dbTopRarity = document.getElementById('db-top-rarity-icon');
        if (rarityIcon && dbTopRarity) dbTopRarity.src = rarityIcon.src;
        
        const typeIcon = document.querySelector('.typing-icon');
        const dbTopType = document.getElementById('db-top-type-icon');
        ...
        const spinDial = document.getElementById('db-spin-dial');
        ...
        if (ezaContainer && ezaContainer.style.display !== 'none') {
            ...
            if (dbEzaImg && awakeningSrc) { dbEzaImg.src = awakeningSrc; dbEzaImg.style.display = 'block'; }
            if (dbTopEzaImg && awakeningSrc) { dbTopEzaImg.src = awakeningSrc; dbTopEzaImg.style.display = 'block'; }
        } else {
            if (dbEzaImg) dbEzaImg.style.display = 'none';
            if (dbTopEzaImg) dbTopEzaImg.style.display = 'none';
        }

        const dbRarityIconRight = document.getElementById('db-rarity-icon');
        if (dbRarityIconRight && rarityIcon) dbRarityIconRight.src = rarityIcon.src;
        
        const dbTypeIconRight = document.getElementById('db-type-icon');
        if (dbTypeIconRight && typeIcon) dbTypeIconRight.src = typeIcon.src;
        
        const myOverlay = document.getElementById('myOverlayImage');
        const dbArtImg = document.getElementById('db-art-img');
    ...
    try {
        ...
        const dbPassiveName = document.getElementById('db-passive-name');
        if (dbPassiveName) dbPassiveName.innerHTML = "<span>Passive Skill - <i>" + passiveName + "</i></span>";
        
        const dbPassiveCont = document.getElementById('db-passive-container');
    ...
    try {
        if (window.updateDokkanDBSuperAttacks) {
    ...
    // 10. DB AWAKENINGS & TRANSFORMATIONS SYNC
    try {
        ...
        if (awakenCont) {
            ...
            if (activeRarity === 'TUR' || activeRarity === 'LR') {
                ...
                awHTML += `
                    ...
                    <div class="db-awaken-row">
                        ${buildDbCardIcon(turSrc, './images/rarity_TUR.png', false)}
                        <div class="db-awaken-date" style="font-size: 15px; font-weight: bold; text-align: center; flex: 1;">Release Date:<br><span style="font-weight: normal; font-size: 13.5px;">${baseDate}</span></div>
                    </div>
                `;
            }
            ...
            if (activeRarity === 'LR') {
                ...
                awHTML += `
                    ...
                    <div class="db-awaken-row">
                        ${buildDbCardIcon(lrSrc, './images/rarity_LR.png', false)}
                        <div class="db-awaken-date" style="font-size: 15px; font-weight: bold; text-align: center; flex: 1;">Release Date:<br><span style="font-weight: normal; font-size: 13.5px;">${baseDate}</span></div>
                    </div>
                `;
            }
            ...
            if (activeAwakening === 'eza' || activeAwakening === 'seza') {
                ...
                const maxRar = activeRarity === 'LR' ? './images/rarity_LR.png' : './images/rarity_TUR.png';
                awHTML += `
                ...
                forms.forEach((f, idx) => {
                    ...
                    trHTML += `
                        <div class="db-transform-row">
                            ${buildDbCardIcon(fImg, activeRarity === 'LR' ? './images/rarity_LR.png' : './images/rarity_TUR.png', false)}
