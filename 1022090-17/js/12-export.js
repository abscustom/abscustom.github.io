...
// SMART CHARACTER FIRST NAME EXTRACTOR
function extractCharacterFirstName(rawName) {
    if (!rawName) return "";
    let clean = rawName.replace(/[\(\)\[\]\{\}\&\+\,\.]/g, ' ').trim();
    let words = clean.split(/\s+/).filter(w => w.length > 0);
    if (words.length === 0) return "";

    const skipWords = ["super", "ultra", "extreme", "mastered", "grand", "great", "king", "dr", "saint", "mr", "mrs", "ms", "instinct", "saiyan"];
    let targetWord = words[0];
    
    for (let w of words) {
        let low = w.toLowerCase().replace(/[^a-z0-9]/g, '');
        if (low && !skipWords.includes(low)) {
            targetWord = low;
            break;
        }
    }

    return targetWord.toLowerCase().replace(/[^a-z0-9]/g, '');
}
...
window.openUploadModal = function() {
    ...
    const firstName = extractCharacterFirstName(charNameRaw);

    const finalFolderId = firstName ? `${baseId}-${firstName}` : baseId;
