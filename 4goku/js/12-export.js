...
// UPLOAD TO GITHUB (FULL CARD PUBLISHER WITH SELECTIVE BLOB & FORM ASSET EXPORT)
window.uploadToGitHub = async function() {
    ...
    try {
        ...
        // 1. COLLECT SCRAPED & FORM ASSET BLOBS FROM MEMORY
        ...
        // 2. TARGET ONLY BLOBS AND BASE64 DATA URLS (FORM CUTINS, USER UPLOADS, ABS THUMBNAILS)
        ...
        for (let idx = 0; idx < cloneImgs.length; idx++) {
            ...
            if (exportName) {
                ...
                if (blob) {
                    ...
                    const fileName = `cutin_${idx + 1}_${Date.now().toString(36)}.${ext}`;
                    const path = `${basePath}/images/${fileName}`;
                    if (!fileMap.has(path)) {
                        filesToUpload.push({ path, blob });
                        fileMap.set(path, true);
        ...
        // 3. SCAN BASE FRAMEWORK SCRIPTS & STYLES
        ...
        // 4. FORCE INCLUDE BASE REPOSITORY IMAGE FRAMEWORK ASSETS
