...
// UPLOAD TO GITHUB (FULL CARD PUBLISHER WITH ALL ASSETS & METADATA)
window.uploadToGitHub = async function() {
    ...
    try {
        ...
        // 1. BAKE LIVE VALUES & CALCULATED STATS INTO DOM BEFORE CLONING
        window.updateIdentity();
        window.applyCardTheme(currentType);
        window.applyAwakening(currentAwakeningMode);
        window.calcFromMin('hp');
        ...
        // Bake attribute values directly onto DOM input elements
        savedInputs.forEach(id => {
            const el = document.getElementById(id);
            if (el) {
        ...
        // Reset quick save button on clone so it's clean on published pages
        ...
        // INJECT PUBLISHED METADATA & INITIAL CARD STATE IN SCRIPT MARKER
        ...
        // Mark body element with is-published class for pure CSS rules
        ...
        // BAKE INLINE CSS VARIABLES ON ABS LAYOUT
        ...
        // 1. COLLECT SCRAPED & FORM ASSETS FROM MEMORY
        ...
        // 2. SYSTEMATICALLY SCAN ALL LIVE DOM IMAGES & CONVERT ALL CUTINS/BLOBS TO FILES
        const liveImgs = document.querySelectorAll('img');
        const cloneImgs = clone.querySelectorAll('img');

        for (let idx = 0; idx < liveImgs.length; idx++) {
            const liveImg = liveImgs[idx];
            const cloneImg = cloneImgs[idx];
            if (!cloneImg) continue;

            const liveSrc = liveImg.src || liveImg.getAttribute('src') || '';
            const exportName = liveImg.getAttribute('data-export-name') || cloneImg.getAttribute('data-export-name');

            let targetFileName = '';
            if (exportName) {
                targetFileName = exportName.replace(/^images\//, '');
            }

            let imageBlob = null;

            if (targetFileName && window.scrapedAssets && window.scrapedAssets[targetFileName]) {
                imageBlob = window.scrapedAssets[targetFileName];
            }

            if (!imageBlob && liveSrc) {
                if (liveSrc.startsWith('data:')) {
                    imageBlob = await dataUrlToBlob(liveSrc);
                } else if (liveSrc.startsWith('blob:') || liveSrc.startsWith('http')) {
                    try {
                        const res = await fetch(liveSrc);
            ...
            if (imageBlob) {
                if (!targetFileName) {
                    ...
                    targetFileName = `img_${idx + 1}_${Date.now().toString(36)}.${ext}`;
                }

                const path = `${basePath}/images/${targetFileName}`;
                if (!fileMap.has(path)) {
                    filesToUpload.push({ path, blob: imageBlob });
                    fileMap.set(path, true);
                }
                cloneImg.setAttribute('src', `images/${targetFileName}`);
            }
        }

        const assetPaths = new Set();

        // 3. SCAN ALL DOM LINK, SCRIPT, & IMAGE SOURCES
        ...
        // 4. FORCE INCLUDE ESSENTIAL BASE FRAMEWORK, STAT & TYPE ASSETS
        const baseFrameworkAssets = [
            ...
            `images/sp_skill_icon_01.png`,
            `images/sp_skill_icon_02.png`,
            `images/sp_skill_icon_04.png`,
            `images/sp_skill_icon_etc.png`,
            `images/st_0001.png`,
            `images/st_0002.png`,
            `images/st_0011.png`,
            `images/st_0012.png`,
            ...
            `images/st_0102.png`,
            `images/st_1009.png`,
            `images/st_atk_super.png`,
            `images/st_critical_up.png`,
            `images/st_evasion.png`,
            `images/st_recover.png`,
            `images/st_recover_minus.png`,
            `images/pot_skill_02_on.png`,
        ...
        // 5. SCAN ALL ACTIVE CATEGORY IMAGES
        document.querySelectorAll('#card-category-container img, #abs-category-container img').forEach(img => {
            const src = img.getAttribute('src');
        ...
        // 6. SCAN ALL SA STAT IMAGES
        document.querySelectorAll('.sa-block img, #abs-sa-container img').forEach(img => {
            const src = img.getAttribute('src');
            if (src && !src.startsWith('http') && !src.startsWith('data:') && !src.startsWith('blob:')) {
                assetPaths.add(src);
            }
...
window.saveQuickEditToGitHub = async function() {
    ...
    try {
        ...
        // Safeguard identity, release dates, and stats before cloning
        window.updateIdentity();
        window.calcFromMin('hp');
        window.calcFromMin('atk');
        window.calcFromMin('def');
        ...
        // Reset quick save button in clone to default state so it never stays stuck as "Saving..."
        const cloneQuickSave = clone.querySelector('#admin-quick-save-btn');
        if (cloneQuickSave) {
            cloneQuickSave.innerText = "💾 Save Quick Edit";
            cloneQuickSave.style.display = "none";
        ...
        // Process any new cutins / blob images uploaded during quick edit
        const filesToUpload = [];
        const fileMap = new Map();
        const liveImgs = document.querySelectorAll('img');
        const cloneImgs = clone.querySelectorAll('img');
        ...
        for (let idx = 0; idx < liveImgs.length; idx++) {
            ...
            if (imageBlob) {
                if (!targetFileName) {
                    const ext = imageBlob.type.includes('png') ? 'png' : 'jpg';
                    targetFileName = `img_quick_${idx + 1}_${Date.now().toString(36)}.${ext}`;
                }

                const path = `${folderName}/images/${targetFileName}`;
                if (!fileMap.has(path)) {
                    filesToUpload.push({ path, blob: imageBlob });
                    fileMap.set(path, true);
                }
        ...
        for (let i = 0; i < filesToUpload.length; i++) {
            const file = filesToUpload[i];
            try {
                await uploadFileToGitHub(token.trim(), owner, repo, file.path, file.blob, `Quick Edit Image: ${file.path.split('/').pop()}`);
            } catch (e) {}
        }

        // Ensure published state marker script remains intact
        let pubScript = clone.querySelector('#pub-site-marker');
        if (!pubScript) {
            pubScript = document.createElement('script');
            pubScript.id = 'pub-site-marker';
