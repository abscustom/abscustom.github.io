...
window.applyCardTheme = function(newSuffix) {
    ...
    // UPDATE ALL LIGHTNING OVERLAYS (BOTH MAIN & DB LAYOUT)
...
window.updateIconImages = function() {
    ...
    // Live sync directly to Dokkan DB Layout icons
    if (window.syncToDbLayout) {
        window.syncToDbLayout();
    }
};
...
window.updateRarityStats = function(rarityName) {
    ...
    // Handle the bottom preview rows
    if (rarityName === "none") {
...
window.autoGenerateSAIcons = function() {
    ...
    if (critCustomMatch) {
        statsToAdd.push({ img: "./images/pot_skill_02_on.png", val: critCustomMatch[1] + "%", target: "self", turns: "1 turn" });
    } else if (t.includes("great chance of performing a critical hit")) {
        statsToAdd.push({ img: "./images/pot_skill_02_on.png", val: "70%", target: "self", turns: "1 turn" });
    } else if (t.includes("high chance of performing a critical hit")) {
        statsToAdd.push({ img: "./images/pot_skill_02_on.png", val: "50%", target: "self", turns: "1 turn" });
    } else if (t.includes("chance of performing a critical hit")) {
        statsToAdd.push({ img: "./images/pot_skill_02_on.png", val: "30%", target: "self", turns: "1 turn" });
    } else if (t.includes("performs a critical hit")) {
        statsToAdd.push({ img: "./images/pot_skill_02_on.png", val: "100%", target: "self", turns: "1 turn" });
    }
    ...
    // --- 7. RENDER STAT ROWS WITH DATA ATTRIBUTES ---
    let htmlBuffer = "";
...
// HELPER FUNCTION: FORMATS QUOTED CATEGORIES IN LEADER SKILL WITH GLOW
window.formatLeaderSkillQuotes = function(text) {
    if (!text) return "";
    return text.replace(/["“”]([^"”]+)["”]/g, '<span class="db-category-quote">"$1"</span>').replace(/\n/g, '<br>');
};

window.updateIdentity = function() {
    const rawDesc = document.getElementById("descInput")?.value || "";
    const name = document.getElementById("nameInput")?.value || "";
    const leaderRaw = document.getElementById("leaderInput")?.value || "";
    ...
    // SYNC TO DOKKAN DB LAYOUT WITH GLOWING QUOTES
    const dbDesc = document.getElementById("db-char-description");
    ...
    const dbLeader = document.getElementById("db-leader-skill");
    if(dbLeader) dbLeader.innerHTML = window.formatLeaderSkillQuotes(leaderRaw);

    const dbPassiveName = document.getElementById("db-passive-name");
    if(dbPassiveName) dbPassiveName.textContent = "Passive Skill - " + (document.getElementById("input-passive-name-sidebar")?.value || "");

    const container = document.getElementById("release-dates-container");
    const baseDate = document.getElementById("dateInput")?.value || "";
    const ezaDate = document.getElementById("ezaDateInput")?.value || "";
    const sezaDate = document.getElementById("sezaDateInput")?.value || "";
