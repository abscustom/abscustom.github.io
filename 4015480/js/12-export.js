...
window.executeGitHubUpload = async function() {
    ...
    try {
        ...
        // PURGE LOADER, HUD, & MODALS FROM PUBLISHED CARDS (Ensure we don't accidentally delete quicksave/admin modals!)
        const toRemove = [
...
`;
        ...
        // Delay alert slightly so user can see checkmark
        setTimeout(() => {
...
window.executeQuickSave = async function() {
    ...
    try {
        ...
        // Keep quicksave and admin modal active in clone so you can still edit it later
        const toRemove = [
            '#uploadGithubBtn', '#topbar-upload-dock-wrap', '#icon-picker-modal', 
            '#glass-upload-modal', 
            '#main-autosave-indicator', '#hud-loading-spinner', '#editor', 
