...
window.toggleCardTheme = function(isDbTheme) {
    ...
    if (isDbTheme) {
        if (appEl) appEl.classList.add('theme-abs-style');
        ...
        if (appEl) appEl.classList.remove('theme-abs-style');
...
let absSaRenderTimer = null;

window.updateAbsStyleSuperAttacks = function() {
    if (absSaRenderTimer) cancelAnimationFrame(absSaRenderTimer);

    absSaRenderTimer = requestAnimationFrame(() => {
...
function enforceAbsPipes() {
    const el = document.getElementById('abs-passive-name');
    if (!el) return;
...
document.addEventListener('input', (e) => {
    if (e.target && e.target.id === 'input-passive-name-sidebar') {
        setTimeout(enforceAbsPipes, 10);
    }
});

window.addEventListener('load', () => {
    setTimeout(enforceAbsPipes, 100);
});
